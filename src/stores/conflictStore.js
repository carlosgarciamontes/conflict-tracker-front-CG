  ...
  actions: {
    async fetchConflicts() {
      ...
      try {
        const response = await fetch(
          `${import.meta.env.VITE_API_URL}/api/v1/conflicts`,
        );
    ...
    async addConflict(newConflict) {
      try {
        ...
        if (response.ok) {
          const savedConflict = await response.json();

          this.conflicts.push(savedConflict);
          return true;
        }
        throw new Error("Error al guardar en el servidor");
      } catch (err) {
        console.error("No se pudo añadir el conflicto:", err);
        return false;
      }
    },
    ...
    async deleteConflict(id) {
      ...
      try {
        const response = await fetch(
          `${import.meta.env.VITE_API_URL}/api/v1/conflicts/${id}`,
          {
            method: "DELETE",
          },
        );
